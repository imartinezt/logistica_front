...
def render_candidates(data):
    ...
    coloreando los candidatos seleccionados para la entrega
    """
    st.subheader("🏆 Rutas candidatas")
    ...
    if alternatives:
        ...
        st.subheader("🔎 Rutas evaluadas")
        complete_alternatives = data.get('complete_options', [])

        if complete_alternatives:

            df_alternatives_complete = pd.DataFrame(complete_alternatives)
            styled_df_complete = df_alternatives_complete.style.apply(highlight_selected_row, axis=1)

            if 'costo' in df_alternatives_complete.columns:
                df_alternatives_complete['costo'] = df_alternatives_complete['costo'].apply(format_currency)
            if 'score' in df_alternatives_complete.columns:
                df_alternatives_complete['score'] = df_alternatives_complete['score'].apply(format_percentage)

            st.dataframe(styled_df_complete, use_container_width=True, column_config=column_configuration)
